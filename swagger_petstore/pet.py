...
def test_post_new_pet():
    ...
    response = requests.post(
        URN + "/pet", headers=HEADERS, data=json.dumps(payload), timeout=15
    )
...
def test_post_existing_pet():
    ...
    response = requests.post(
        URN + f"/pet/{PET_ID}", headers=local_header, data=payload, timeout=15
    )
    # print(response.json())  # Print the response in json format
    assert response.status_code == 200, "Error: " + str(response.status_code)
...
def test_get_pets_status():
    ...
    response = requests.get(
        URN + "/pet/findByStatus?status=sold", headers=HEADERS, timeout=15
    )
    assert response.status_code == 200, "Error: " + str(response.status_code)
...
def test_get_pets_tags():
    ...
    response = requests.get(
        URN + "/pet/findByTags?tags=noisy", headers=HEADERS, timeout=15
    )
    assert response.status_code == 200, "Error: " + str(response.status_code)
...
def test_delete_pet():
    ...
    response = requests.delete(URN + f"/pet/{PET_ID}", headers=HEADERS, timeout=15)
    assert response.status_code == 200, "Error: " + str(response.status_code)
